...
Vertex_Index :: ModelShaderVertex
...
parse_obj_path :: proc(path: string, base_dir: string = "", flags := Flags{}) -> (o: OBJ) {
	file_data, err := os.read_entire_file(path, context.temp_allocator)
	if err != nil {
		log.errorf("TINYOBJ :: Failed to load \"{}\": {}", path, err)
		return
	}

	return parse_obj(string(file_data), base_dir, flags)
}
...
*/
parse_obj :: proc(buf: string, base_dir: string = "", flags := Flags{}) -> (o: OBJ) {
	...
	for line in strings.split_lines_iterator(&it) {
		...
		case "f":
			...
			for i := 1; i < len(parts); i += 1 {
				vi: Vertex_Index
				vi.pos = INVALID_INDEX
				vi.texcoord = INVALID_INDEX
				vi.normal = INVALID_INDEX
				...
				if len(slashes) >= 1 && len(slashes[0]) > 0 {
					vi.pos = fix_index(parse_int(slashes[0]), v_count)
				}
				if len(slashes) >= 2 && len(slashes[1]) > 0 {
					vi.texcoord = fix_index(parse_int(slashes[1]), vt_count)
				}
				if len(slashes) >= 3 && len(slashes[2]) > 0 {
					vi.normal = fix_index(parse_int(slashes[2]), vn_count)
				}
				append(&face_indices, vi)
			}
			// Triangulation Logic
			if (.triangulate in flags) && len(face_indices) > 3 {
				i0 := face_indices[0]
				// Fan triangulation
		...
		case "mtllib":
			if len(parts) >= 2 {
				...
				if len(base_dir) > 0 {
					...
					full_path, alloc_err = filepath.join({base_dir, fname}, context.temp_allocator)
			...
			if len(parts) >= 2 {
				...
				if face_count > prev_shape_face_offset {
					commit_shape(
						&o,
						current_shape_name,
						prev_shape_face_offset,
						face_count - prev_shape_face_offset,
					)
